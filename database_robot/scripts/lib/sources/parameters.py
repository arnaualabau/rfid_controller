#!/usr/bin/env python
import rospy
#from self_exceptions import InvalidParameter
...
class ParameterHelper:
    ...
    def section_to_dict_list(self, values):
        ...
        if self._parameter_exist(values.qr_section):
            section["qr_section"] = values.qr_section
        if self._parameter_exist(values.map_path):
            section["map_path"] = values.map_path
        if self._parameter_exist(values.description):
            section["description"] = values.description
        if self._parameter_exist(values.id_shop):
            section["id_shop"] = int(values.id_shop)
        if self._parameter_exist(values.waypoint_path):
            section["waypoint_path"] = values.waypoint_path
        if section:
            res.append(section)
